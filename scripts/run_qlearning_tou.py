"""
Train Q-learning agents for TOU retail price arbitrage (Model B).

Identical structure to run_qlearning.py but uses ActewAGL Home Daytime
...
from src.dp.prices import build_tou_profile 
...
# ---- Setup ----
prices = build_tou_profile()                           # TOU prices instead of spot
load_profile = generate_load_profile()
solar_profile = generate_solar_profile()
dss_file = FEEDER_32['dss_file']

print("=" * 70)
print("  Q-LEARNING TRAINING: ActewAGL TOU Retail Prices (Model B)")
print("=" * 70)
print(f"  Solar soak: {prices[22]:.0f} A$/MWh (11am-3pm)")
print(f"  Shoulder:   {prices[0]:.0f} A$/MWh")
print(f"  Peak:       {prices[14]:.1f} A$/MWh (7-9am, 5-9pm)")
print(f"  Spread:     {max(prices) - min(prices):.1f} A$/MWh")

# Q-learning hyperparameters (same defaults as spot training)
n_soc_bins = 80
n_actions = 21
n_phase1 = 50000
n_phase2 = 50000
...
# ---- Sensitivity grid ----
dispatch_limits = [100] # [50, 80, 100]
capacities = [300, 400] # [200, 300, 400]
all_results = []
...
for limit in dispatch_limits:
    for cap in capacities:
        ...
        s_dp = timeseries.summarise(df_dp, f"TOU DP ±{limit}kW {cap}kWh")

        # ---- Q-Learning (two-phase, warm-started from TOU DP) ----
        print(f"\n  --- Q-Learning (two-phase, warm-started from TOU DP) ---")
        ...
        # Normal: full two-phase training
        ...
        #     label=f"tou_{limit}kW_{cap}kWh")

        # Skip Phase 1: reuse existing Phase 1 Q-table
        Q, history = train_two_phase(
            env_phase1, env_phase2,
            n_phase1=n_phase1, n_phase2=n_phase2,
            n_soc_bins=n_soc_bins,
            ...
            save_dir='data/q_tables',
            label=f"tou_{limit}kW_{cap}kWh",
            qtable_phase1=f"data/q_tables/Q_phase1_tou_{limit}kW_{cap}kWh.npz")
        ...
        df_rl = timeseries.run(rl_dispatch, load_profile, solar_profile,
                               FEEDER_32, dss_file, battery_enabled=True, battery=battery)
        s_rl = timeseries.summarise(df_rl, f"TOU RL ±{limit}kW {cap}kWh")
...
print(f"\n{'='*90}")
print(f"  TOU DP vs TOU Q-LEARNING: HEAD-TO-HEAD COMPARISON")
print(f"  Price: ActewAGL Home Daytime Economy (GST excl.)")
print(f"{'='*90}")
...
# ---- Improvement Table ----
print(f"\n{'='*90}")
print(f"  RL IMPROVEMENT OVER DP (TOU prices)")
print(f"{'='*90}")
...
for r in all_results:
    ...
    if r['rl_violations'] == 0 and r['dp_violations'] > 0:
        ...
        if abs(rev_gap) < 1:
            ...
            assessment = "RL feasible, less revenue"
...
if dp_feasible:
    ...
    print(f"  TOU DP minimum feasible:  ±{dp_min['limit']}kW / {dp_min['capacity']}kWh "
          f"(${dp_min['dp_revenue']:.2f}/day)")
else:
    print(f"  TOU DP: no fully feasible configuration in tested range")

if rl_feasible:
    rl_min = min(rl_feasible, key=lambda r: (r['capacity'], r['limit']))
    print(f"  TOU RL minimum feasible:  ±{rl_min['limit']}kW / {rl_min['capacity']}kWh "
          f"(${rl_min['rl_revenue']:.2f}/day)")
else:
    print(f"  TOU RL: no fully feasible configuration in tested range")

# ---- Dispatch Profiles (only where DP and RL differ) ----
for r in all_results:
    ...
    print(f"\n{'='*90}")
    print(f"  DISPATCH PROFILE: ±{limit}kW / {cap}kWh (TOU prices)")
    ...
    for t in range(48):
        ...
        print(f"  {t:<4} {h:02d}:{m:02d}  ${prices[t]:>6.1f}  {dp_str:>7} {dp_soc[t]:>7.1f}  "
              f"{rl_str:>7} {rl_soc[t]:>7.1f}  {period}")
